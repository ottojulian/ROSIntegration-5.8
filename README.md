## Unreal Engine 5.8 Plugin Migration Changes

This fork contains the following changes required to compile ROSIntegration with Unreal Engine 5.8.

### `TArray::SetNumUninitialized()` API Change

Unreal Engine 5.8 changed the second parameter of `TArray::SetNumUninitialized()` from `bool` to `EAllowShrinking`.

In `Source/ROSIntegration/Private/rosbridge2cpp/TCPConnection.cpp`, the following calls:
...
This fixes the UE 5.8 compilation error:
```
cannot convert argument 2 from 'bool' to 'EAllowShrinking'
```
### LookAtScanComponent Callback
UE 5.8's stricter compile-time format-string validation caused errors when the SubscribeCallback lambda was initialized directly inside LookAtScanComponent.h.

The callback was changed from an inline initialization:
```
std::function<void(TSharedPtr<FROSBaseMsg>)> SubscribeCallback =
    [this](TSharedPtr<FROSBaseMsg> msg) -> void
    {
        // ...
    };
```
to a declaration in the header:
```
std::function<void(TSharedPtr<FROSBaseMsg>)> SubscribeCallback;
```
The lambda is now initialized in the ULookAtScanComponent constructor in LookAtScanComponent.cpp:
```
...
```
This resolved the following UE 5.8 compilation error:
```
UE::Core::Private::FormatStringSan::TAtArgPos:
template parameter 'N': 'UCFS_Result':
a variable with non-static storage duration cannot be used as a non-type argument
```

Floating-Point Literal Warnings
MSVC generated C4305 warnings for the following declarations:
```
float FixedUpdateInterval = 0.01666666667;
```
The literal was interpreted as a double and implicitly converted to float.

The declarations were changed to explicitly use a float literal:
```
...
Source/ROSIntegration/Classes/ROSBridgeParamOverride.h

This removes the corresponding C4305 warnings.

### Build Result
After applying the changes, ROSIntegration successfully compiled with:
...
No changes were required to the bundled RapidJSON implementation or BSON dependencies for the successful UE 5.8 build.
---
